...
% fprintf('!!!!!! This is the SpatialFrequency Domain Experiment!!!!!!\n');
...
switch Name
    ...
    case 'CIFAR10_YCbCr'
        FileName=strcat(CIFAR10_YCbCr_DataPath,'_Batch',int2str(optionsData.BatchNumber),'.mat');
        load(FileName);
        ImageDim=[32 32 3];
        NumberOfBatches=optionsData.TotalNumberOfBatches;
        options{1}.NumberOfBatches=NumberOfBatches;
...
elseif strcmp(optionsData.WhichData,'Features') %precomputed features  
    ...
    if strcmp(options{2}.Type,'ExtractBRIEF')
                        savedir=strcat(RootPath,'\BRIEF\BriefOverlappedPatches\Saved BRIEF_OverlappedPatches Features\',options{9}.Domain,options{9}.SampleNo,optionsData.TrainTest,'-');
                        if strcmp(optionsData.TrainTest,'Train') %optionsData.TrainTest='Train'
                                if options{9}.SmoothingFlag
                                        FileName=strcat(savedir,int2str(options{1}.TotalNumberOfBatches),';',optionsData.DatasetName,'_',options{9}.Dimension,'_',options{9}.ColorType,'_[',int2str(options{9}.SmoothKernel),']_',num2str(options{9}.SD_SmoothingFilter),'_BRIEF_Batch_',int2str(options{1,1}.BatchNumber),'.mat');
                                else
                                        FileName=strcat(savedir,int2str(options{1}.TotalNumberOfBatches),';',optionsData.DatasetName,'_',options{9}.Dimension,'_',options{9}.ColorType,'_[NO Smoothing]_BRIEF_Batch_',int2str(options{1,1}.BatchNumber),'.mat');
                                end
...
%                                     Dataset=Features;
    ...
    elseif strcmp(options{2}.Type,'ExtractPoolNormalizeBinaryFeature')
        if strcmp(optionsData.TrainTest,'Train')
                FilePrefix=strcat('FeaturesPooledNormalized_Binary_Train_',int2str(optionsData.BatchNumber),'_');
                FileName=strcat(fullfile(optionsNormalize.SavePath,strcat(FilePrefix,optionsData.DatasetName,'_',int2str(options{8}.BitsPerRFs),'Bits_NoRFs',int2str(options{8}.NumberOfRFs),'_RFSize',int2str(options{8}.rfSize),'_Sp',int2str(int8(optionsExtract.SparsifyFlag)),...
                    '_SMp',int2str(100*optionsExtract.SparsityMultiplier),'_',optionsPool.SummationType,'_Bin',int2str(int8(optionsNormalize.Binarize)))),'.mat');
                %autoload function. If file does not exist, decrement the stage (Features to Raw, Expanded Feature to Features etc) and try loading again.
                    if exist(FileName, 'file')
                        load(FileName);
                    ...
                    %no need for autoload check. If the train file is created then,
                    %test file also exist (unless deleted).
                    FileName=fullfile(optionsNormalize.SavePath,strcat(FilePrefix,optionsData.DatasetName,'_',int2str(options{8}.BitsPerRFs),'Bits_NoRFs',int2str(options{8}.NumberOfRFs),'_RFSize',int2str(options{8}.rfSize),'_Sp',int2str(int8(optionsExtract.SparsifyFlag)),'_SMp',int2str(100*optionsExtract.SparsityMultiplier),'_',optionsPool.SummationType,'_Bin',int2str(int8(optionsNormalize.Binarize))));        
                    if exist(FileName, 'file')
                        load(FileName);
                        if optionsData.Verbose 
                           disp('Features Loaded Test (No computation)') 
