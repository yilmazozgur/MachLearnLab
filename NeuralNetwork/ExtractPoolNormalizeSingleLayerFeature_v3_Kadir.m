...
%
% V3 Update (22.07.2015): Binarization options are enriched. 
%
%From:
%TOU_ML
...
function NeuralNetFeatures=ExtractPoolNormalizeSingleLayerFeature_v3_Kadir(Dataset,ReceptiveFields,DataStats,options)
...
%FIND HIDDEN LAYER ACTIVITIES, START
    ...
    for j=i:1:i+optionsExtract.BatchSize-1
        ...
        if ReceptiveFields.ImageIsGrayFlag
            ...
            elseif Dataset.options{1, 8}.TwoChannelFlag
    ...
    xx = sum(patches.^2, 2);%squares of patches; we will use it in distance computation; (x-y)^2=x^2+(y^2-2*x*y)==> xx=y
%     cc = sum(centroids.^2, 2)';
    xc = patches * centroids'; %"xc=xx*cc"
