function [qq]=SamplingFrom_FrequencyPhase(Dataset,X,options)

PatchSize=options{9}.PatchSize3;
stride=options{9}.Stride3;
Number_Of_Tests=options{9}.NumberOfTests3;
RootPath=options{10};
...
%     case 'RGB'
        ...
        if strcmp(Dimension,'3D')
            phase_=zeros(1,Number_Of_Tests(1));
        end;
%         freq_=zeros(1,Number_Of_Tests(1));
        for img=1:size(X,1)
                    I=X(img,:);
...
%                         if strcmp(Dimension,'3D')
                            ...
                            phaseI1_1=freqPhaseI(Ind1_1);
                            phaseI1_2=freqPhaseI(Ind1_2); 

                            phase_(:)=not(phaseI1_1 < phaseI1_2); phase_=double(phase_(:));
                            q( Number_Of_Tests(1)*(j-1)+1 : Number_Of_Tests(1)*j )  = phase_;
