function [qq]=SamplingFrom_FrequencyMagnitude(Dataset,X,options)



PatchSize=options{9}.PatchSize2;
stride=options{9}.Stride2;
Number_Of_Tests=options{9}.NumberOfTests2;
RootPath=options{10};
...
%     case 'RGB'
        ...
        if strcmp(Dimension,'3D')
            magnitude_=zeros(1,Number_Of_Tests(1));
        end;
...
%                         if strcmp(Dimension,'3D')
                            ...
                            magnitudeI1_1=freqMagnitudeI(Ind1_1);
                            magnitudeI1_2=freqMagnitudeI(Ind1_2); 

                            magnitude_(:)=not(magnitudeI1_1 < magnitudeI1_2); magnitude_=double(magnitude_(:));
                            q( Number_Of_Tests(1)*(j-1)+1 : Number_Of_Tests(1)*j )  = magnitude_;
